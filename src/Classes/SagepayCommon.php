...
class SagepayCommon
{
    protected static $nonSensitiveRequestDataArray = ["VPSProtocol", "TxType", "Vendor", "VendorTxCode", "Amount","Currency", "Description"];
    protected static $nonSensitiveResponseDataArray = ["VPSProtocol", "Status", "StatusDetail", "VPSTxId"];
     ...
     */
    public static function vendorTxCode($orderId, $type = false, $prefix = false)
    {
        $parts = [];
        // add prefix
        if (!empty($prefix)) {
     ...
     */
    public static function vendorTxCode2OrderId($vendorTxCode)
    {
        $orderId = false;
     ...
     * @return array
     */
    private static function _getCardDetails($creditCard)
    {
        $query = [];
     ...
     * @return array
     */
    private static function _populateDeliveryDetails(
        SagepayCustomerDetails $billingDetails,
    ...
    private static function _setAuxValue(array $query, $key, $value)
    {
        if (!empty($value)) {
            $query[$key] = $value;
        }
     ...
     * @return string
     */
    private static function _lookupCardType($cardType)
    {
        switch ($cardType) {
            case 'mastercard':
                return 'MC';
