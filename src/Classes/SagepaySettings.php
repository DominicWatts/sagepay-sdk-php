...
class SagepaySettings
{
    ...
    private $_purchaseUrls = [
        ...
        'live' => [
            ...
            'direct' => SAGEPAY_DIRECT_SERVER_LIVE,
     ...
     */
    public static function getInstance(array $config = [], $loadFileConfig = true)
    {
    ...
    {
        if (in_array($env, [SAGEPAY_ENV_TEST, SAGEPAY_ENV_LIVE])) {
            $this->_env = $env;
        } else {
    ...
    {
        if (in_array($accountType, [SAGEPAY_ACCOUNT_ECOMMERCE, SAGEPAY_ACCOUNT_CONTINUOUS, SAGEPAY_ACCOUNT_MAIL])) {
    ...
    {
        if (in_array($serverProfile, [SAGEPAY_SERVER_PROFILE_LOW, SAGEPAY_SERVER_PROFILE_NORMAL])) {
    ...
    {
        if (in_array($billingAgreement, [0, 1], true)) {
